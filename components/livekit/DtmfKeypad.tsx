    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={cn(
            'bg-gray-800 border border-gray-700 absolute bottom-full left-0 mb-2 rounded-2xl p-4 shadow-lg',
            'z-50 min-w-[280px]',
            className
          )}
        >
            ...
            {DTMF_KEYS.map((key) => (
              <Button
                ...
                className={cn(
                  ...
                  pressedKeys.has(key.value) && 'bg-blue-600 text-white scale-95'
                )}
              >
                <span className="text-xl font-semibold">{key.label}</span>
                {key.subLabel && (
                  <span className="text-gray-400 absolute bottom-1 text-[10px]">
                    {key.subLabel}
                  </span>
                )}
              </Button>
          ...
          {onClose && (
            <Button
              ...
              size="sm"
              onClick={onClose}
              className="text-gray-400 hover:text-white mt-3 w-full"
            >
              Close keypad
            </Button>
          )}
